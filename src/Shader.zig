...
pub fn compile(vertex_source: []const u8, fragment_source: []const u8, geometry_source: ?[]const u8) !Shader {
    var vertex_shader = gl.createShader(gl.VERTEX_SHADER);
    gl.shaderSource(vertex_shader, 1, &vertex_source.ptr, &[_]i32{@intCast(vertex_source.len)});
    gl.compileShader(vertex_shader);
    try checkCompilerError(vertex_shader, ErrorType.vertex);

    var fragment_shader = gl.createShader(gl.FRAGMENT_SHADER);
    gl.shaderSource(fragment_shader, 1, &fragment_source.ptr, &[_]i32{@intCast(fragment_source.len)});
